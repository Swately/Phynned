// ipc/src/PhynnedAgentPublisher.cpp
// PhynnedAgentPublisher — shared memory creation and per-tick publish.
//
...
PhynnedAgentPublisher::open(const char* shm_name, uint32_t agent_pid) noexcept {
    ...
    file_handle_ = CreateFileMappingW(
        ...
        PAGE_READWRITE,
        0u,
        static_cast<DWORD>(kPhynnedShmSize),
        wname);

    if (!file_handle_) {
        std::fprintf(stderr,
            "[PhynnedPublisher] CreateFileMappingW('%s') failed: error=%lu\n",
    ...
    void* view = MapViewOfFile(file_handle_, FILE_MAP_ALL_ACCESS,
                               0u, 0u, kPhynnedShmSize);
    if (!view) {
        std::fprintf(stderr,
            "[PhynnedPublisher] MapViewOfFile failed: error=%lu\n",
            GetLastError());
    ...
    if (shm_fd_ < 0) {
        std::fprintf(stderr, "[PhynnedPublisher] shm_open('%s') failed\n", shm_name_);
        return std::unexpected(phyriad::Error{phyriad::ErrorCode::SystemError});
    }

    if (ftruncate(shm_fd_, static_cast<off_t>(kPhynnedShmSize)) < 0) {
        ::close(shm_fd_);
        shm_fd_ = -1;
    ...
    void* view = mmap(nullptr, kPhynnedShmSize,
    ...
    std::memset(layout_, 0, sizeof(PhynnedShmHeader) + sizeof(PhynnedStateHeader));

    layout_->header.version = kPhynnedShmVersion;
    ...
    layout_->header.magic = kPhynnedShmMagic;

    action_read_cursor_ = 0ull;

    std::fprintf(stdout,
        "[PhynnedPublisher] SHM created: '%s'  size=%.1f KB  pid=%u\n",
        shm_name,
        static_cast<double>(kPhynnedShmSize) / 1024.0,
        agent_pid);
    return {};
}

// ── publish() ────────────────────────────────────────────────────────────────
void PhynnedAgentPublisher::publish(
...
// ── set_bench_phase() ─────────────────────────────────────────────────────────
void PhynnedAgentPublisher::set_bench_phase(uint8_t phase) noexcept {
    if (!layout_) return;
...
void PhynnedAgentPublisher::set_hw_classification(uint8_t cpu_class,
                                                uint8_t ccd_count,
...
// ── set_policies_paused() ─────────────────────────────────────────────────────
void PhynnedAgentPublisher::set_policies_paused(uint8_t paused) noexcept {
    if (!layout_) return;
    // Single-byte write is atomic on x86/ARM — no seqlock needed.
    // The UI's PhynnedLogicNode reads this without holding any lock.
...
// ── mark_disconnected() ───────────────────────────────────────────────────────
void PhynnedAgentPublisher::mark_disconnected() noexcept {
    if (!layout_) return;
    shm_write_begin(&layout_->header);
...
// ── close() ──────────────────────────────────────────────────────────────────
void PhynnedAgentPublisher::close() noexcept {
    if (!layout_) return;
    ...
    layout_ = nullptr;
    std::fprintf(stdout, "[PhynnedPublisher] SHM closed.\n");
}
