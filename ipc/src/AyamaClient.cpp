// apps/ayama/ipc/src/AyamaClient.cpp
// AyamaClient — shared memory connect/disconnect.
//
...
AyamaClient::connect(const char* shm_name) noexcept {
    ...
    void* view = MapViewOfFile(file_handle_, access, 0, 0, kAyamaShmSize);
    if (!view) {
    ...
    void* view = mmap(nullptr, kAyamaShmSize, prot, MAP_SHARED, shm_fd_, 0);
    if (view == MAP_FAILED) {
        close(shm_fd_);
        shm_fd_ = -1;
        return std::unexpected(phyriad::Error{phyriad::ErrorCode::SystemError});
