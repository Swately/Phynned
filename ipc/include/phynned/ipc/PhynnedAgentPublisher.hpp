// ipc/include/phynned/ipc/PhynnedAgentPublisher.hpp
// PhynnedAgentPublisher — write side of the agent's shared memory segment.
//
// Creates the named FileMapping (Windows) / POSIX shared memory and publishes
// TargetProcess[], TargetMetrics[], PolicyDecision[], and PhynnedStateHeader
...
class PhynnedAgentPublisher {
public:
    PhynnedAgentPublisher()  noexcept = default;
    ~PhynnedAgentPublisher() noexcept { close(); }

    PhynnedAgentPublisher(PhynnedAgentPublisher const&)            = delete;
    PhynnedAgentPublisher& operator=(PhynnedAgentPublisher const&) = delete;
    PhynnedAgentPublisher(PhynnedAgentPublisher&&)                 = delete;
    PhynnedAgentPublisher& operator=(PhynnedAgentPublisher&&)      = delete;

    // ── Lifecycle ─────────────────────────────────────────────────────────

    /// Create the named shared memory and initialise the header.
    /// `agent_pid` is written into PhynnedShmHeader::agent_pid.
    ...
    [[nodiscard]] PhynnedCommandSlot* command_slot() noexcept {
