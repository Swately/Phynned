// apps/ayama/ipc/include/ayama/ipc/AyamaProtocol.hpp
// AyamaProtocol — shared memory layout between ayama-agent and UI clients.
//
// Layout (1 MB total):
//   [0..127]            AyamaShmHeader   (magic, version, agent_pid, seqlock)
//   [128..255]          AyamaStateHeader (counts, flags)
...
//                                            = 384 + 7168 = 7552 B)
//   [20608..20671]      AyamaCommandSlot   (64 B — UI→agent IPC commands)
...
// don't have hardcoded offsets. If client compatibility breaks, bump kAyamaShmVersion.
//
// Seqlock pattern:
...
inline constexpr uint32_t kAyamaShmMagic   = 0x41594D41u;  // 'AYMA'
inline constexpr uint32_t kAyamaShmVersion = 1u;
inline constexpr uint64_t kAyamaShmSize    = 1u * 1024u * 1024u;  // 1 MB
...
static_assert(sizeof(AyamaShmHeader) == 128);

/// High-level counts / flags (128 bytes).
struct alignas(64) AyamaStateHeader {
    ...
    // so the user can verify Ayama isn't burning their CPU.
...
static_assert(sizeof(AyamaStateHeader) == 128);
static_assert(offsetof(AyamaStateHeader, policies_paused) == 52);
static_assert(offsetof(AyamaStateHeader, self_cpu_pct)    == 56);
static_assert(offsetof(AyamaStateHeader, self_rss_mb)     == 60);
static_assert(offsetof(AyamaStateHeader, ccd_defense_count)   == 40);
static_assert(offsetof(AyamaStateHeader, ccd_defense_cpu_pct) == 44);
static_assert(offsetof(AyamaStateHeader, cpu_class)           == 48);
static_assert(offsetof(AyamaStateHeader, ccd_count)           == 49);
...
//
// kAyamaCmdNone is the initial state. Commands are idempotent — re-issuing
...
// holds because typically only one Ayama UI instance runs per session.
enum AyamaCmdKind : uint32_t {
    kAyamaCmdNone                  = 0u,
    kAyamaCmdPausePolicies         = 1u,  ///< Revert all active, suspend new applies
    kAyamaCmdResumePolicies        = 2u,  ///< Re-enable applies
    kAyamaCmdForceRevertAll        = 3u,  ///< Revert all but do NOT suspend (single-shot)
    kAyamaCmdSetDifferentialPin    = 4u,  ///< Toggle Rule 7 differential pin mode.
                                          ///<  arg1 = 0 → disable (default Rule 1).
                                          ///<  arg1 = 1 → enable (differential pin).
};

struct alignas(64) AyamaCommandSlot {
    std::atomic<uint64_t> seq;            //  8B  — bumped by client on submit
    std::atomic<uint64_t> ack;            //  8B  — bumped by agent after processing
    uint32_t              cmd_kind;       //  4B  — AyamaCmdKind
    uint32_t              target_pid;     //  4B  — optional argument
    uint64_t              arg1;           //  8B
    uint64_t              arg2;           //  8B
    uint8_t               _pad[24];       // 24B  — fill to 64B
};
static_assert(sizeof(AyamaCommandSlot) == 64);

/// Complete shared memory layout — accessed via pointer to mapped region.
struct AyamaShmLayout {
    AyamaShmHeader   header;                                      // 128B @ 0
    AyamaStateHeader state;                                       // 128B @ 128
    ...
    AyamaCommandSlot         command_slot;                        //  64B @ 20608
    // Baseline ring fills the rest of the 1 MB mapping.
};
// Offsets updated for kMaxTargets=64.
// command_slot offset added.
static_assert(offsetof(AyamaShmLayout, targets)      ==    256u);
static_assert(offsetof(AyamaShmLayout, metrics)      ==   4352u);
static_assert(offsetof(AyamaShmLayout, decisions)    ==  12544u);
static_assert(offsetof(AyamaShmLayout, command_slot) ==  20608u);

// ── Seqlock helpers ───────────────────────────────────────────────────────
/// Begin a write transaction: seq becomes odd.
inline void shm_write_begin(AyamaShmHeader* h) noexcept {
...
inline void shm_write_end(AyamaShmHeader* h) noexcept {
    h->seq.fetch_add(1u, std::memory_order_acq_rel);  // HAL: acq_rel fetch_add — synchronising counter
}
/// Read the state header consistently (retry up to 4 times).
/// Returns true if a consistent read was obtained.
inline bool shm_read_consistent(const AyamaShmHeader* h,
                                 const AyamaStateHeader* src,
                                 AyamaStateHeader* dst) noexcept {
