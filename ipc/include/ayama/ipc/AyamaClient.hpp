// apps/ayama/ipc/include/ayama/ipc/AyamaClient.hpp
// AyamaClient — read-only view into the agent's shared memory.
//
// Used by standard_window and other UI clients to display agent data.
// Connect to the named shared memory created by ayama-agent.
...
class AyamaClient {
public:
    AyamaClient() noexcept = default;
    ~AyamaClient() noexcept { disconnect(); }

    AyamaClient(AyamaClient const&)            = delete;
    AyamaClient& operator=(AyamaClient const&) = delete;

    /// Open the shared memory created by ayama-agent.
    /// `shm_name` must match AgentConfig::shm_name (default "Local\\AyamaAgent.v1").
    [[nodiscard]] std::expected<void, phyriad::Error>
    connect(const char* shm_name = "Local\\AyamaAgent.v1") noexcept;
    ...
    [[nodiscard]] const AyamaStateHeader* state() const noexcept {
