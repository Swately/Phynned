// observer/include/phynned/observer/ClassificationCache.hpp
                                             ...
                                             uint64_t           now_tsc) noexcept
    {
        ...
        for (uint32_t i = 0u; i < n_entries_; ++i) {
            ...
            if (e.pid == pid) {
                ...
                // Order of "strength" (most specific → most useful for Phynned):
                ...
                if (kind_changed) {
                    ...
                    std::fprintf(stdout,
                        "[Phynned][Classify] %-32s [pid=%u] | %s → %s | "
        ...
        if (k != TargetKind::Unknown) {
            ...
            std::fprintf(stdout,
                "[Phynned][Classify] %-32s [pid=%u] → %-12s | "
                "fs=%d fg=%us cpu=%.1f%% threads=%u d3d=%d\n",
                exe_name, pid, kname,
                fresh_info.is_fullscreen ? 1 : 0,
                fresh_info.foreground_for_sec,
