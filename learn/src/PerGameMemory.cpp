...
std::expected<void, phyriad::Error> PerGameMemory::save(const char* path) noexcept
{
    ...
    std::fprintf(f,
        "# Phynned per-game learned optimisations\n"
        "# Auto-generated by phynned-agent — edit while agent is stopped.\n"
    ...
    if (n_bad_ > 0u) {
        std::fprintf(f,
            "# Bad-list: Phynned detected regression for these exes.\n"
            "# Remove an entry to allow Phynned to retry.\n\n");
...
void PerGameMemory::mark_bad(const char* exe, const char* reason) noexcept
{
    ...
    std::fprintf(stdout,
        "[Phynned][Learn] Marked %s as bad: %s\n",
        exe, b.reason);
}
...
uint32_t PerGameMemory::expire_stale_entries(uint32_t max_age_days) noexcept
{
    ...
    for (uint32_t i = 0u; i < n_entries_; ++i) {
        ...
        std::fprintf(stdout,
            "[Phynned][Learn] Expired stale entry: %s (>%u days)\n",
            e.exe, max_age_days);
    }
    return expired;
}
