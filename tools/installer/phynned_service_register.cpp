// tools/installer/phynned_service_register.cpp
// phynned-service-register — Windows service registration helper.
//
// Registers phynned-agent.exe as a Windows Service that starts automatically.
// Must be run as Administrator. This is an alternative to install.bat for
// programmatic installation from a GUI wizard or CI.
//
// Usage:
//   phynned-service-register install   [--path <exe_path>]
//   phynned-service-register uninstall
//   phynned-service-register status
//   phynned-service-register start
//   phynned-service-register stop
//
// The service name is "PhynnedAgent". Display name: "Phynned Runtime Optimizer".
//
#include <cstdio>
#include <cstring>
#include <cstdlib>
...
static constexpr const char* kServiceName    = "PhynnedAgent";
static constexpr const char* kDisplayName    = "Phynned Runtime Optimizer";
static constexpr const char* kDescription    = "Phynned background optimizer. "
...
static bool get_default_exe_path(char* out, DWORD max) noexcept {
    ...
    if (last_sep) {
        *(last_sep + 1) = '\0';
        strncat_s(out, max, "phynned-agent.exe", _TRUNCATE);
    }
    return true;
}
...
static int do_install(const char* exe_path) {
    ...
    std::fprintf(stdout,
        ...
        "To start immediately: phynned-service-register start\n"
...
static int do_install(const char*) {
    std::fprintf(stderr, "Service registration is Windows-only.\n"
                         "On Linux, create a systemd unit for phynned-agent.\n");
    return 1;
}
...
static void print_usage() {
    std::fprintf(stdout,
        "phynned-service-register v0.1\n"
        "Register phynned-agent.exe as a Windows Service.\n\n"
        "Usage:\n"
        "  phynned-service-register install   [--path <agent.exe>]\n"
        "  phynned-service-register uninstall\n"
        "  phynned-service-register status\n"
        "  phynned-service-register start\n"
        "  phynned-service-register stop\n\n"
