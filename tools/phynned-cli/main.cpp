// tools/phynned-cli/main.cpp
// phynned-cli — command-line interface for phynned-agent control and analysis.
//
// Commands:
//   status                          Show agent status (connected / disconnected)
//   targets                         List currently observed targets
//   actions                         Show recent action log
//   bench diff <a> <b>              Compare two phynned-bench CSV outputs
//   presentmon-import <pm.csv>      Parse PresentMon CSV → phynned-bench format
...
static int cmd_status()
{
    phynned::ipc::PhynnedClient client;
    const auto r = client.connect();
    if (!r.has_value()) {
        std::fprintf(stdout,
            "Status: DISCONNECTED\n"
            "  Agent not running or SHM unavailable.\n"
            "  Launch phynned-agent.exe to enable optimization.\n");
        return 1;
    }

    const phynned::ipc::PhynnedStateHeader* state = client.state();
    if (!state) {
...
static int send_ipc_command_(uint32_t cmd_kind, const char* label) {
    ...
    std::fprintf(stdout, "[phynned-cli] %s sent (seq=%llu). Waiting ack...\n",
                 label, static_cast<unsigned long long>(seq));
    for (int i = 0; i < 200; ++i) {  // 200 × 10ms = 2 s
        if (client.command_ack() >= seq) {
            std::fprintf(stdout, "[phynned-cli] Agent ack'd.\n");
            return 0;
        }
#ifdef _WIN32
        Sleep(10);
    ...
    std::fprintf(stderr,
        "[phynned-cli] WARN: no ack within 2 s. Command may still be applied;\n"
        "  check `phynned-cli status` and `phynned-cli actions`.\n");
    return 2;
}

static int cmd_pause() {
    return send_ipc_command_(phynned::ipc::kPhynnedCmdPausePolicies,
                             "pause policies");
}
static int cmd_resume() {
    return send_ipc_command_(phynned::ipc::kPhynnedCmdResumePolicies,
                             "resume policies");
}
static int cmd_revert_all() {
    return send_ipc_command_(phynned::ipc::kPhynnedCmdForceRevertAll,
                             "force revert all");
}
                            ...
                            const std::vector<const char*>& treateds) noexcept
{
    ...
    std::fprintf(stdout,
        "\n-- phynned-bench multi-run aggregate -----------------------------\n"
...
static int cmd_memory_list()
{
    ...
    if (lad) std::snprintf(memory_path, sizeof(memory_path), "%s\\Phynned\\memory.toml", lad);
#else
    const char* home = ::getenv("HOME");
    if (home) std::snprintf(memory_path, sizeof(memory_path), "%s/.config/phynned/memory.toml", home);
#endif
...
#ifdef _WIN32
        const char* lad = ::getenv("LOCALAPPDATA");
        if (lad) std::snprintf(path, sizeof(path), "%s\\Phynned\\memory.toml", lad);
#else
        const char* home = ::getenv("HOME");
        if (home) std::snprintf(path, sizeof(path), "%s/.config/phynned/memory.toml", home);
#endif
...
static int cmd_memory_clear_bad()
{
    ...
    std::fprintf(stdout,
        "Cleared %u bad-list entries. Phynned will retry all previously "
        "regressed exes.\n", n);
    return 0;
}
...
#ifdef _WIN32
    const char* lad = ::getenv("LOCALAPPDATA");
    if (lad) std::snprintf(path, sizeof(path), "%s\\Phynned\\audit.bin", lad);
#else
    const char* home = ::getenv("HOME");
    if (home) std::snprintf(path, sizeof(path), "%s/.config/phynned/audit.bin", home);
#endif
    if (path[0] == '\0') {
        std::fprintf(stderr, "Error: cannot determine config directory.\n");
        return 1;
...
// ── presentmon-import — convertir CSV de PresentMon a formato phynned-bench ──
//
...
//   5. Escribe output en formato phynned-bench (key,value lines).
//
                                  ...
                                  const char* output) noexcept
{
    ...
    std::fprintf(stdout,
        "[phynned-cli] Header parsed. PresentMon format detected:\n"
    ...
    // ── 5. Write phynned-bench-format CSV ───────────────────────────────────
    if (output) {
        ...
        std::fprintf(out,
            "# phynned-cli presentmon-import output\n"
            "# source=%s\n"
            "# filter_process=%s\n"
            "# filter_pid=%u\n"
            "metric,value\n"
        ...
        std::fprintf(stdout,
            "Next:    phynned-cli bench diff <baseline.csv> <treated.csv>\n");
    }
    return 0;
}
...
{
    std::fprintf(stdout,
        "phynned-cli v0.2 — Phynned command-line interface\n\n"
        "Usage: phynned-cli <command> [args...]\n\n"
        "Commands:\n"
        ...
        "  bench diff <a> <b>                    Compare two CSV files (phynned-bench / presentmon-import format)\n"
        ...
        "  presentmon-import <pm.csv>            Convert PresentMon CSV → phynned-bench CSV\n"
        "      [--process <name>]                  Filter rows by Application name (case-insensitive prefix)\n"
        "      [--pid <uint>]                      Filter rows by ProcessID\n"
        "      [--output <file>]                   Output path (default: <pm>.phynned.csv)\n"
        ...
        "  2) Enable Phynned policy (run phynned-agent as Administrator)\n"
        "  3) Capture treated:\n"
        "       PresentMon.exe --process_name javaw.exe --output_file treated.pm.csv\n"
        "  4) Convert both:\n"
        "       phynned-cli presentmon-import baseline.pm.csv --process javaw.exe --output baseline.csv\n"
        "       phynned-cli presentmon-import treated.pm.csv  --process javaw.exe --output treated.csv\n"
        "  5) Diff:\n"
        "       phynned-cli bench diff baseline.csv treated.csv\n");
}
...
int main(int argc, char** argv)
{
    ...
    if (std::strcmp(cmd, "bench") == 0) {
        ...
        std::fprintf(stderr,
            "Usage:\n"
            "  phynned-cli bench diff <baseline.csv> <treated.csv>\n"
            "  phynned-cli bench multi --baseline <f1.csv> [--baseline <f2.csv> ...]\n"
    ...
    if (std::strcmp(cmd, "presentmon-import") == 0) {
        if (argc < 3) {
            std::fprintf(stderr,
                "Usage: phynned-cli presentmon-import <pm.csv> "
        ...
        if (!output) {
            std::snprintf(default_out, sizeof(default_out), "%s.phynned.csv", pm_csv);
            output = default_out;
        }
