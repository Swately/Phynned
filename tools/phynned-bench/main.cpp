// tools/phynned-bench/main.cpp
// phynned-bench — standalone synthetic benchmark for Phynned validation.
//
// Simulates a CPU-bound game-like workload. Designed for A/B testing:
//   Without Phynned: run → capture baseline.csv
//   With Phynned:    run → capture treated.csv → compare with phynned-cli diff.
//
// Usage:
//   phynned-bench [--duration <ms>] [--output <file>] [--seed <uint>]
//
...
static void print_usage() noexcept {
    std::fprintf(stdout,
        "phynned-bench v0.1 — Phynned synthetic benchmark\n\n"
        "Usage: phynned-bench [options]\n\n"
        "Options:\n"
        ...
        "Example (A/B test):\n"
        "  phynned-bench --output baseline.csv      # without Phynned\n"
        "  # Start phynned-agent, wait for detection\n"
        "  phynned-bench --output treated.csv       # with Phynned active\n"
        "  phynned-cli diff baseline.csv treated.csv\n");
}
...
int main(int argc, char** argv)
{
    ...
    if (!args.quiet) {
        std::fprintf(stdout,
            "phynned-bench: duration=%u ms  seed=0x%X  output=%s\n",
            args.duration_ms, args.seed, args.output);
        std::fprintf(stdout,
            "Running workload — do NOT change process affinity during baseline.\n"
            "If Phynned is active, it will detect and pin this process.\n\n");
    }

    // ── Run workload ────────────────────────────────────────────────────
    const auto wall_t0 = std::chrono::steady_clock::now();

    phynned_bench::WorkloadRunner runner(args.duration_ms, args.seed);
    const phynned_bench::WorkloadResult result = runner.run();
    ...
    // ── Write CSV (for phynned-cli diff) ──────────────────────────────────
