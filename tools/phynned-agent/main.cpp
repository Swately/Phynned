// tools/phynned-agent/main.cpp
// phynned-agent — Phynned background optimizer daemon.
//
...
#include <phynned/core/Diag.hpp>          // crash-diagnostic phase markers
#include <phynned/core/SingleInstance.hpp>
...
static phynned::core::AgentRuntime* g_runtime = nullptr;
...
static LONG WINAPI phynned_unhandled_filter(EXCEPTION_POINTERS* eps) noexcept {
    if (!eps || !eps->ExceptionRecord) {
        std::fprintf(stderr, "[Phynned][CRASH] (no exception record)\n");
        std::fflush(stderr);
    ...
    const uint32_t phase = phynned::core::diag::g_last_phase
                            .load(std::memory_order_relaxed);  // HAL: relaxed — secondary atomic in compound op
    const uint32_t tick  = phynned::core::diag::g_last_tick
                            .load(std::memory_order_relaxed);  // HAL: relaxed — secondary atomic in compound op
    const uint32_t ap_pid  = phynned::core::diag::g_last_apply_pid
                                .load(std::memory_order_relaxed);  // HAL: relaxed — secondary atomic in compound op
    const uint32_t ap_rule = phynned::core::diag::g_last_apply_rule
                                .load(std::memory_order_relaxed);  // HAL: relaxed — secondary atomic in compound op
    const uint32_t ap_tid  = phynned::core::diag::g_last_apply_tid
                                .load(std::memory_order_relaxed);  // HAL: relaxed — secondary atomic in compound op

    std::fprintf(stderr,
        "\n[Phynned][CRASH] code=0x%08lx (%s) addr=%p\n"
        "[Phynned][CRASH] tick=%u last_phase=%u (%s)\n"
        "[Phynned][CRASH] last_apply: pid=%u rule=%u tid=%u\n",
        static_cast<unsigned long>(code),
        exception_code_name(code),
        addr,
        tick, phase, phynned::core::diag::phase_name(phase),
    ...
    if (code == EXCEPTION_ACCESS_VIOLATION && er->NumberParameters >= 2u) {
        ...
        const char* kind =
            ...
            (rw_kind == 8) ? "DEP/NX" : "?";
        std::fprintf(stderr,
            "[Phynned][CRASH] AV detail: %s at 0x%llx\n",
...
static BOOL WINAPI console_ctrl_handler(DWORD ctrl_type) noexcept {
    if (ctrl_type == CTRL_C_EVENT || ctrl_type == CTRL_BREAK_EVENT ||
        ...
        if (g_runtime) {
            std::fprintf(stdout, "\n[Phynned] Shutdown signal received — stopping...\n");
            g_runtime->stop();
        }
        return TRUE;
    }
...
int main(int argc, char* argv[]) {
    ...
    SetUnhandledExceptionFilter(phynned_unhandled_filter);
#endif

    const CliArgs cli = parse_args(argc, argv);

    std::fprintf(stdout,
        "Phynned Agent v0.1.0\n"
        "Built on Phyriad Framework 1.0.0\n"
        "-----------------------------------------------\n");

    // ── Single instance guard ─────────────────────────────────────────────
    phynned::core::SingleInstance inst;
    if (!inst.acquire()) {
        std::fprintf(stderr,
            "[Phynned] Another instance is already running. Exiting.\n");
        return 1;
    }
    ...
    if (!start_result) {
        std::fprintf(stderr,
            "[Phynned] Failed to start: error code %d\n",
    ...
    std::fprintf(stdout,
        "[Phynned] Running. Press Ctrl+C to stop.\n"
    ...
    std::fprintf(stdout,
        "[Phynned] Clean shutdown. Ticks: %u\n",
        runtime.tick_count());
