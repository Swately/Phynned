// apps/ayama/tools/ayama-ui/AyamaLogicNode.hpp
// Logic node for ayama-ui: connects the Phyriad UI to the agent via IPC.
//
// on_start() — attempts to connect AyamaClient. Calculates reconnect interval.
// tick()     — if connected: reads SHM → compresses to AyamaSnapshotMini → publishes.
//              if not: retries reconnect every ~1s (rate-limited by TSC).
//
// Multi-inlet: InputEvent (idx 0) + WindowState (idx 1).
// Single outlet: AyamaAppState (idx 0).
//
...
// Privilege: none — AyamaClient only opens an existing SHM mapping.
#pragma once
#include "AyamaAppState.hpp"
#include "AyamaTrayIcon.hpp"
...
public:
    ...
    [[nodiscard]] phyriad::node::Outlet<AyamaAppState>& outlet() noexcept {
        return out_state;
    }

    // ── Client access (for draw_widgets / panels) ────────────────────────────
    [[nodiscard]] ayama::ipc::AyamaClient& client() noexcept { return client_; }
    [[nodiscard]] const ayama::ipc::AyamaClient& client() const noexcept { return client_; }
    ...
    [[nodiscard]] std::expected<void, phyriad::Error> tick() noexcept {
        ...
        //    AyamaAppState stores these as win_width/win_height (same value sin HiDPI;
        ...
        // Read extended state from AyamaStateHeader.
        if (const auto* st = client_.state()) {
            ...
            // every ~500 ms). Routes into AyamaAppState's top-level
...
private:
    ...
    static constexpr const wchar_t* kTrayClassName_ = L"AyamaTrayMsgWnd";
        ...
        HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) noexcept
    {
        ...
        AyamaLogicNode* self = nullptr;
        if (uMsg == WM_NCCREATE) {
            const auto* cs = reinterpret_cast<CREATESTRUCTW*>(lParam);
            self = static_cast<AyamaLogicNode*>(cs->lpCreateParams);
            SetWindowLongPtrW(hWnd, GWLP_USERDATA,
                              reinterpret_cast<LONG_PTR>(self));
        } else {
            self = reinterpret_cast<AyamaLogicNode*>(
                GetWindowLongPtrW(hWnd, GWLP_USERDATA));
        }

        if (self) {
            if (uMsg == AyamaTrayIcon::kTrayMsg) {
    ...
    // ── Tray state machine ────────────────────────────────────────────────────
    void update_tray(const AyamaAppState& s) noexcept
    {
        ...
        const ayama_ui::UserSettings& prefs = ayama_ui::current_settings();
        ...
        if (new_state != last_tray_state_) {
            ...
            {
                tray_.show_toast(L"Ayama", L"Agent connected and monitoring.");
            }
#else
            (void)old_state;
#endif
        ...
        if (s.snap.agent_connected && prefs.toast_game_detected) {
            for (const auto& t : client_.targets()) {
                ...
                tray_.show_toast(title,
                    L"Game detected - Ayama is optimizing it.");
