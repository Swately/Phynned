// apps/ayama/tools/ayama-ui/widgets/policies_panel.hpp
// Policies panel - list of active rules and decisions from the current cycle.
//
// Reads decisions[] directly from AyamaClient.
...
inline void draw_policies_panel(const AyamaAppState& s,
                                 const ayama::ipc::AyamaClient* ac) noexcept
{
    ...
    if (ac == nullptr || !s.snap.agent_connected) {
        ...
        ImGui::TextWrapped("Ayama agent is not running. "
                           "Launch ayama-agent.exe to see active policies.");
        ImGui::PopStyleColor();
        return;
    }
        ...
        ImVec2{0.f, ImGui::GetTextLineHeightWithSpacing() * 8.f}))
    {
        ...
        for (const auto& d : decisions) {
            ...
            // d.action_kind es ayama::policy::ActionKind (enum class uint8_t)
