// apps/ayama/tools/ayama-ui/widgets/dashboard_panel.hpp
...
//
// Contract:
//   - Receives AyamaAppState (published by AyamaLogicNode each frame)
//   - Receives AyamaClient* for direct SHM array access in panels
...
inline void draw_dashboard_panel(const AyamaAppState& s,
                                  const ayama::ipc::AyamaClient* ac) noexcept
{
    using namespace ayama_ui;
    ...
    //
    // Safety-first: Ayama defaults to PAUSED. Nothing is applied to any
    // process until the user explicitly clicks Start. This protects users
    // who launch Ayama out of curiosity while a kernel-anticheat game is
    // running — the CPU-affinity changes Ayama would otherwise make are
    // gated behind this opt-in.
    //
    // Commands go through AyamaClient::send_command(), which writes to a
    ...
    if (ac && ac->is_connected()) {
        ...
        if (ImGui::Button("Start", ImVec2{110.f, 36.f})) {
            (void)const_cast<ayama::ipc::AyamaClient*>(ac)
                ->send_command(ayama::ipc::kAyamaCmdResumePolicies);
        }
        ...
        if (ImGui::Button("Pause", ImVec2{110.f, 36.f})) {
            (void)const_cast<ayama::ipc::AyamaClient*>(ac)
                ->send_command(ayama::ipc::kAyamaCmdPausePolicies);
        }
        ImGui::PopStyleColor(3);
        ...
        if (ImGui::Button("Reset", ImVec2{110.f, 36.f})) {
            (void)const_cast<ayama::ipc::AyamaClient*>(ac)
                ->send_command(ayama::ipc::kAyamaCmdForceRevertAll);
        }
        ImGui::PopStyleColor(3);
        ...
        if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip(
                ...
                "Use after the Start button if something feels off — Ayama\n"
    ...
    if (ac == nullptr || !s.snap.agent_connected) {
        ...
        ImGui::TextUnformatted("Welcome to Ayama");
        ImGui::SetWindowFontScale(1.0f);
        ImGui::PopStyleColor();
        ImGui::Spacing();
        ImGui::TextWrapped(
            "The Ayama agent is not currently running. "
            "Once you launch ayama-agent.exe, this dashboard refreshes "
            "automatically.");
        ImGui::Spacing();
        ImGui::Separator();
        ImGui::Spacing();
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Hero card - what Ayama is doing, right now, in plain language
    // ─────────────────────────────────────────────────────────────────────
    {
        ...
        ImGui::SameLine();
        help_marker(
            "Ayama auto-detects your CPU topology at startup. The strategy "
            "below is chosen based on which asymmetric features are present "
            "(V-Cache CCD, P/E cores, or multi-CCD). For symmetric CPUs, "
            "Ayama monitors but does not change process affinity.");

        ImGui::TextWrapped("Strategy: %s", what_ayama_does(s.snap.cpu_class));
    ...
    // games with an active action (cross-referencing AyamaClient targets
    ...
    // doesn't confuse "Ayama is idle" with "Ayama is broken".
    // ─────────────────────────────────────────────────────────────────────
    if (ac && ac->is_connected()) {
        ...
        for (const auto& t : targets) {
            if (t.kind != ayama::observer::TargetKind::Game) continue;
        ...
        if (shown == 0u) {
            ...
            if (s.snap.policies_paused) {
                ...
                ImGui::TextUnformatted(
                    "No games detected yet. Launch one and Ayama "
    ...
    // ─────────────────────────────────────────────────────────────────────
    {
        ...
        help_marker(
            ...
            "Ayama can still observe - but cannot apply policy changes to "
        ...
        ImGui::SameLine();
        help_marker(
            "Auto: Ayama applies policies automatically when it detects a "
            "qualifying process.\n"
            "Assist: same detection, but Ayama asks before applying.\n"
    ...
    ImGui::Indent();
    {
        ...
        if (s.snap.bad_count > 0u) {
            ...
            help_marker(
                ...
                "regression. Ayama auto-reverts and bad-lists them so it "
        ...
        if (is_asymmetric(s.snap.cpu_class)) {
            ...
            help_marker(
                ...
                "thrash, 1.5+ = severe (Ayama actively re-pins).");
            ImGui::SameLine();
        ...
        if (has_vcache(s.snap.cpu_class)) {
            if (s.snap.ccd_defense_count > 0u) {
                ...
                help_marker(
                    ...
                    "to hurt your game, Ayama evicts them. Only fires on "
    ...
    if (ImGui::CollapsingHeader("Advanced details")) {
        ...
        ImGui::SameLine();
        help_marker(
            "Ayama's own CPU usage. Budget: <0.3% when idle, <1.0% when "
