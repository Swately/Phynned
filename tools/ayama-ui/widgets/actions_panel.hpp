// apps/ayama/tools/ayama-ui/widgets/actions_panel.hpp
// Actions panel — history of applied and reverted actions.
//
// Reads action_log[] directly from AyamaClient (SPMC ring buffer).
...
inline void draw_actions_panel(const AyamaAppState& s,
                                const ayama::ipc::AyamaClient* ac) noexcept
{
    ...
    if (ac == nullptr || !s.snap.agent_connected) {
        ...
        ImGui::TextWrapped("Ayama agent is not running. "
                           "Launch ayama-agent.exe to see action history.");
        ImGui::PopStyleColor();
        return;
    }
    ...
    ImGui::TextDisabled(
        ...
        "'ayama-cli revert --all' to undo every active policy.");
    ...
    if (log.empty()) {
        ...
        ImGui::Spacing();
        ImGui::TextDisabled(
            "Actions appear here when Ayama pins a process "
