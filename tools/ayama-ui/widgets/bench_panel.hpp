// apps/ayama/tools/ayama-ui/widgets/bench_panel.hpp
// Benchmark panel - A/B/A/B/A test runner.
//
// Integrates PresentMon capture, ayama-cli import, and agent pause/resume
...
// One BenchRunner per ayama-ui process is sufficient (single-window app).
inline ayama::ui::BenchRunner& bench_runner_instance() noexcept {
    static ayama::ui::BenchRunner inst;
    return inst;
}
...
inline void bench_paths_resolve_(BenchPanelPaths& p) noexcept {
    ...
    // `runtime/` next to ayama-cli + ayama-agent. Older flat builds
    // (where PresentMon was a sibling of ayama-ui.exe) still work via
    // candidate (2).
    {
    ...
    // ayama-cli: search the same canonical locations as PresentMon
    ...
    // %TEMP%\ayama-bench. Either way, the directory is created if missing.
    const auto& settings = ayama_ui::current_settings();
    if (settings.bench_output_dir[0] != '\0') {
        ...
        if (tn > 0u && tn < sizeof(tmp)) {
            ...
            std::strncpy(p.out_dir, "C:\\Temp\\ayama-bench",
                         sizeof(p.out_dir) - 1u);
            CreateDirectoryA(p.out_dir, nullptr);
        }
    }
...
inline void draw_bench_panel(const AyamaAppState& s,
                              const ayama::ipc::AyamaClient* ac) noexcept
{
    ...
    // owns the underlying AyamaClient via AyamaLogicNode; the const here is
    // for the read-only views. Cast is safe in single-window UI context.)
    auto* writable_client = const_cast<ayama::ipc::AyamaClient*>(ac);
    runner.set_presentmon_path(paths.pm_exe);
    runner.set_ayama_cli_path(paths.cli_exe);
    ...
    if (ac == nullptr || !s.snap.agent_connected) {
        ...
        ImGui::TextWrapped("Ayama agent is not running. "
    ...
    if (!paths.pm_ok || !paths.cli_ok) {
        ...
        if (!paths.pm_ok) {
            ...
            ImGui::TextWrapped("Re-run CMake configure with internet access "
                               ...
                               "next to ayama-ui.exe manually.");
        }
        if (!paths.cli_ok) {
            ImGui::Text("ayama-cli.exe NOT FOUND at: %s", paths.cli_exe);
        }
        ImGui::PopStyleColor();
        ImGui::Separator();
    }
    ...
    ImGui::TextWrapped(
        ...
        "ayama-cli bench multi computes 95%% CIs and significance.");
    ImGui::PopStyleColor();
    ImGui::Separator();

    // ── State indicator ────────────────────────────────────────────────────
    const auto rstate = runner.state();
    ImGui::PushStyleColor(ImGuiCol_Text, ayama::ui::bench_state_color(rstate));
    ImGui::Text("State: %s", ayama::ui::bench_state_label(rstate));
    ImGui::PopStyleColor();
    if (rstate != ayama::ui::BenchRunner::State::Idle) {
        ImGui::SameLine();
    ...
    if (rstate == ayama::ui::BenchRunner::State::Idle ||
        rstate == ayama::ui::BenchRunner::State::Done ||
        rstate == ayama::ui::BenchRunner::State::Failed)
    {
        ImGui::Spacing();
        ...
        // to the V-Cache CCD.
        //
        // Toggle is sent to agent via IPC command (kAyamaCmdSetDifferentialPin).
                                 ...
                                 &s_diff_pin_enabled))
            {
                if (writable_client) {
                    (void)writable_client->send_command(
                        ayama::ipc::kAyamaCmdSetDifferentialPin,
                        /*target_pid=*/0u,
        ...
        if (rstate == ayama::ui::BenchRunner::State::CapturingRun) {
            char buf[64];
        ...
        for (uint32_t i = 0u; i < runner.n_runs(); ++i) {
            ...
            if (r.completed && r.import_exit_code == 0) {
                ...
                ImGui::TextColored(ImVec4{0.95f, 0.30f, 0.25f, 1.f},
                       ...
                       rstate != ayama::ui::BenchRunner::State::Idle &&
                       rstate != ayama::ui::BenchRunner::State::Done) {
    ...
    if (rstate == ayama::ui::BenchRunner::State::Done) {
        const char* log_path = runner.aggregate_log_path();
        if (log_path && log_path[0]) {
            ImGui::Separator();
            ImGui::Text("bench multi output:");
            std::string contents = ayama::ui::read_log_utf8(log_path);
            if (!contents.empty()) {
        ...
        {
            ayama::ui::suggest_report_path(runner.process_name(),
                                           s_export_path,
        ...
        if (ImGui::Button("Export .md")) {
            const bool ok = ayama::ui::generate_md_report(
                runner.runs(),
                runner.n_runs(),
                runner.process_name(),
                runner.protocol(),
    ...
    // ── Error display ──────────────────────────────────────────────────────
    if (rstate == ayama::ui::BenchRunner::State::Failed) {
        ImGui::Separator();
