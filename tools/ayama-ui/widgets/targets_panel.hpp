// apps/ayama/tools/ayama-ui/widgets/targets_panel.hpp
// Targets panel — table of processes observed by Ayama.
//
// Reads targets[] and metrics[] DIRECTLY from AyamaClient (not the Ring)
...
struct OverridesUiState {
    ayama::observer::KindOverrides table{};
    bool                            loaded{false};
...
inline void draw_targets_panel(const AyamaAppState& s,
                                const ayama::ipc::AyamaClient* ac) noexcept
{
    ...
    if (ac == nullptr || !s.snap.agent_connected) {
        ...
        ImGui::TextWrapped("Ayama agent is not running. "
                           "Launch ayama-agent.exe to enable target tracking.");
        ImGui::PopStyleColor();
        return;
    }
    ...
    if (targets.empty()) {
        ...
        ImGui::Spacing();
        ImGui::TextDisabled(
            "Ayama watches for games, streaming apps and comms tools.\n"
                          ...
                          ImVec2{0.f, table_height}))
    {
        ...
        for (std::size_t i = 0u; i < targets.size(); ++i) {
            ...
            // Kind — t.kind es ayama::observer::TargetKind (enum class uint8_t)
    ...
    if (ImGui::CollapsingHeader("Manual classification overrides",
        ...
        for (std::size_t i = 0u; i < targets.size(); ++i) {
            ...
            if (ImGui::Combo("##qov_k", &sel, kKindLabels,
                ...
                ayama::observer::TargetKind nk =
                    static_cast<ayama::observer::TargetKind>(sel);
        ...
        if (ImGui::Button("Add##ov_add")) {
            ov.table.set(ov.new_exe,
                static_cast<ayama::observer::TargetKind>(ov.new_kind_idx));
            (void)ov.table.save();
            ov.new_exe[0] = '\0';
        }
        ...
        if (ov.table.count() == 0u) {
                    ...
                    ImGuiTableFlags_SizingStretchProp))
            {
                ...
                for (uint32_t i = 0u; i < n; ++i) {
                    ...
                    ImGui::TableSetColumnIndex(1);
                    ImGui::TextUnformatted(
                        ayama::observer::kind_to_name(data[i].kind));
                    ImGui::TableSetColumnIndex(2);
        ...
        ImGui::Spacing();
        ImGui::TextDisabled(
            "Stored at: %%LOCALAPPDATA%%\\Ayama\\overrides.txt");
    }
}
