// apps/ayama/tools/ayama-ui/widgets/cpu_class_helpers.hpp
// Shared helpers for rendering arch-aware UI text in ayama-ui.
//
// The agent publishes `AyamaSnapshotMini::cpu_class` (uint8_t) reflecting
...
// ── what_ayama_does() - narrative one-liner per arch ─────────────────────
// Plain-language description of the optimization strategy for this CPU.
[[nodiscard]] inline const char*
what_ayama_does(uint8_t cpu_class) noexcept {
    switch (cpu_class) {
        case kCpuX3DSingle:
        ...
        case kCpuSingleCCD:
            return "Your CPU is symmetric - core pinning has negligible "
                   "benefit. Ayama monitors but does not change affinity.";
        case kCpuUnknown:
        default:
...
// ── is_asymmetric() - does this arch benefit from Ayama? ─────────────────
// SingleCCD and Unknown return false. Used to gate "Ayama is monitoring
// only" messaging.
...
//               "1.5+ = severe - Ayama is actively counteracting.");
...
//
// Per-kind counts come from the AyamaClient targets[] array, so callers
    ...
    char* buf, size_t bufsize) noexcept
{
    ...
    // ("pinned") and everything else that Ayama actively manipulates as
