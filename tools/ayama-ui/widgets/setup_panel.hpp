// apps/ayama/tools/ayama-ui/widgets/setup_panel.hpp
// Setup panel - first-time-user friendly checklist + tutorial.
//
// Goal: a brand-new user opens Ayama, clicks "Setup", and within 30
// seconds knows (a) whether their hardware is supported, (b) whether the
// agent is running with the right privileges, (c) what Ayama will do for
...
inline void draw_setup_panel(const AyamaAppState& s,
                              const ayama::ipc::AyamaClient* ac) noexcept
{
    ...
    ImGui::TextDisabled(
        "Five quick checks so Ayama is set up correctly. Green = good. "
    ...
    // ─────────────────────────────────────────────────────────────────────
    {
        ...
        check_row(
            "1. Agent process",
            ok,
            "ayama-agent.exe is running and publishing telemetry.",
            "ayama-agent.exe is NOT running. Launch it from Start menu "
    ...
    // ─────────────────────────────────────────────────────────────────────
    {
        ...
        if (ok) {
            ...
            ImGui::TextDisabled(
                "Not running as admin. Restart Ayama as administrator "
    ...
    // ─────────────────────────────────────────────────────────────────────
    {
        ...
        ImGui::TextWrapped("%s", what_ayama_does(s.snap.cpu_class));
    ...
    // ─────────────────────────────────────────────────────────────────────
    {
        ...
        check_row(
            ...
            "Agent watchdog detected a stall. Restart ayama-agent.",
            "Ayama runs a self-watchdog: if the main loop doesn't tick "
    ...
    // ─────────────────────────────────────────────────────────────────────
    {
        ...
        ImGui::SameLine();
        help_marker(
            "PresentMon is Intel's frame-capture tool. Ayama bundles it "
        ...
        ImGui::TextDisabled(
            ...
            "PresentMon.exe next to ayama-ui.exe.");
        ImGui::Unindent();
    }
    ...
    if (ImGui::CollapsingHeader("What to do next", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        ImGui::Spacing();
        ImGui::TextDisabled(
            "Tip: Ayama defaults are conservative. If a policy hurts "
