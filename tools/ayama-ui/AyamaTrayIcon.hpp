// apps/ayama/tools/ayama-ui/AyamaTrayIcon.hpp
// AyamaTrayIcon — Windows Shell_NotifyIcon system tray integration.
//
// Manages a notification area icon whose color reflects the agent state:
//   Green  (state 0) — Ayama active, optimisations applied.
//   Yellow (state 1) — Ayama active, Assist mode awaiting confirmation.
...
//   tray.show_toast(L"Ayama", L"Cyberpunk 2077 detected — optimising");
...
enum class TrayState : uint8_t {
    Green  = 0,  ///< Ayama active, optimisations applied.
...
class AyamaTrayIcon {
public:
    AyamaTrayIcon() noexcept = default;
    ~AyamaTrayIcon() noexcept { destroy(); }

    AyamaTrayIcon(const AyamaTrayIcon&)            = delete;
    AyamaTrayIcon& operator=(const AyamaTrayIcon&) = delete;
    ...
    void create(HWND hWnd) noexcept
    {
        ...
        ::wcsncpy(nid_.szTip, L"Ayama — disconnected",
                   ...
                   const wchar_t* tooltip = nullptr) noexcept
    {
        ...
        static const wchar_t* kTips[] = {
            L"Ayama — active (optimising)",
            L"Ayama — Assist mode (awaiting confirmation)",
            L"Ayama — degraded (run as admin for full features)",
            L"Ayama — regression detected, policy reverted",
            L"Ayama — agent not running",
        };
...
private:
    ...
    void show_context_menu() noexcept
    {
        ...
        if (!hMenu) return;
        AppendMenuW(hMenu, MF_STRING,    kMenuOpen, L"Open Ayama");
