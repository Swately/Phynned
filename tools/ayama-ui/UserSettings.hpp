// apps/ayama/tools/ayama-ui/UserSettings.hpp
// UserSettings — UI-side user preferences (separate from agent config).
//
// Stored as plain text key=value at:
//   Windows: %LOCALAPPDATA%\Ayama\settings.txt
//   POSIX:   ~/.config/ayama/settings.txt
//
...
inline std::string settings_file_path() noexcept {
    ...
    const std::string dir = base + "\\Ayama";
...
inline bool save_user_settings(const UserSettings& s) noexcept {
    ...
    if (!f) return false;
    std::fprintf(f,
        "# Ayama UI user settings (auto-generated; safe to edit by hand)\n"
        ...
        "# Empty bench_output_dir = use platform default (%%TEMP%%\\ayama-bench\n"
        "# on Windows, $TMPDIR/ayama-bench on POSIX).\n"
        "bench_output_dir=%s\n",
