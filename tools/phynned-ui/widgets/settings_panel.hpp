// tools/phynned-ui/widgets/settings_panel.hpp
...
// in PhynnedLogicNode; benchmark paths are UI-only).
//
#pragma once
...
// Accessor used by toast triggers in PhynnedLogicNode to gate notifications.
...
inline void draw_settings_panel() noexcept
{
                                 ...
                                 ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        ImGui::SameLine();
        phynned_ui::help_marker(
            "Shows when Phynned classifies a process as a game and starts\n"
        ...
        phynned_ui::help_marker(
            ...
            "is the user-visible signal that Phynned caught itself misfiring.");
        ...
        ImGui::TextDisabled(
            ...
            "Phynned if you want to keep the agent running without any pop-ups.");
        ImGui::Unindent();
        ImGui::Spacing();
    }
                                 ...
                                 ImGuiTreeNodeFlags_DefaultOpen))
    {
        ImGui::Indent();
        ImGui::TextDisabled(
            "Where Phynned writes its benchmark CSV files and reports.");
        ImGui::Spacing();
        ...
        ImGui::TextDisabled(
            "Leave empty for default: %%TEMP%%\\phynned-bench (Windows) or\n"
            "$TMPDIR/phynned-bench (POSIX). The directory is created if it\n"
    ...
    if (ImGui::CollapsingHeader("Config-file locations (read-only)")) {
        ImGui::Indent();
        ImGui::TextDisabled(
            "These files are managed by Phynned and live in your user\n"
            "profile. You can edit them by hand if you know what you're\n"
            "doing; the agent reloads on file-modification.");
        ImGui::Spacing();
        ImGui::BulletText("UI settings: %%LOCALAPPDATA%%\\Phynned\\settings.txt");
        ImGui::BulletText("Policy config: %%LOCALAPPDATA%%\\Phynned\\policies.toml");
        ImGui::BulletText("Kind overrides: %%LOCALAPPDATA%%\\Phynned\\overrides.txt");
        ImGui::BulletText("Per-game memory: %%LOCALAPPDATA%%\\Phynned\\memory.toml");
        ImGui::BulletText("Audit log: %%LOCALAPPDATA%%\\Phynned\\audit.bin");
        ImGui::Unindent();
        ImGui::Spacing();
    }
}
