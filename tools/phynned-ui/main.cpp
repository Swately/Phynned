// tools/phynned-ui/main.cpp
// phynned-ui - standalone Phynned optimizer window.
//
// Follows the standard_window pattern (Phyriad+ImGui reference):
//   - phyriad::ui::Application::run(cfg, build_lambda)
//   - PhynnedLogicNode - logic: reads agent SHM, publishes PhynnedAppState
//   - RenderNode<PhynnedAppState> - render: draw_widgets() with tabs
//   - Wire: "ui"→"logic"→"render"
//
// PhynnedClient (IPC with the agent) lives in PhynnedLogicNode.
...
// Phynned on machines where the iGPU driver is missing or stuck on the
...
// Set before any tick() fires; used by draw_widgets to access PhynnedClient.
// Single-window, single-instance app - this is safe and simple.
// ─────────────────────────────────────────────────────────────────────────────
static PhynnedLogicNode*           g_logic            = nullptr;
static phynned_ui::AgentLauncher*  g_launcher         = nullptr;
...
// ─────────────────────────────────────────────────────────────────────────────
static void draw_widgets(const PhynnedAppState& s) noexcept
{
    ...
    // std::exit because the render lambda has no clean handle on GLFW.
    {
        ...
        const bool modal_open = phynned_ui::draw_anticheat_warning_modal(
    ...
    // Obtain PhynnedClient pointer from logic node (non-owning, always valid
    ...
    if (ImGui::Begin("##phynned_status", nullptr, kBarFlags)) {
        ...
        // tripped. Reads only fields already published by PhynnedLogicNode.
        ...
        if (!connected) {
            ...
            agent_tip   = "No connection to phynned-agent. The UI will keep "
        ...
        // tell at a glance whether Phynned is actively modifying processes
        ...
        if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip(
                "Phynned modifies process priority/affinity at the OS level.\n"
                "Disable Phynned (or exclude the game in Targets) before\n"
        ...
        // Auto-launched indicator shown when phynned-ui spawned its own
        // background agent (vs. user starting phynned-agent.exe manually).
        ...
        // confirm at a glance which strategy Phynned is using.
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4{0.65f, 0.85f, 1.0f, 1.f});
        ImGui::Text("%s", phynned_ui::arch_short_label(s.snap.cpu_class));
        ImGui::PopStyleColor();
        ...
        if (n_games + n_apps + n_watched == 0u) {
            ...
            if (n_games > 0u && phynned_ui::is_asymmetric(s.snap.cpu_class)) {
                ...
                if (ImGui::IsItemHovered()) {
                    ImGui::SetTooltip(
                        ...
                        "  0.8-1.5 - elevated\n"
                        "  >=1.5 - severe (Phynned actively counteracting)");
                }
            }
        }
        ...
        // the PhynnedAppState::self_* fields). Colour-coded against the
        ...
        if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip(
                "Phynned agent's own CPU and resident memory.\n"
    ...
    if (ImGui::Begin("##phynned_main", nullptr, kMainFlags)) {
        if (ImGui::BeginTabBar("##phynned_tabs")) {
            ...
            const bool first_run =
                ...
                s.snap.cpu_class == phynned_ui::kCpuUnknown;
            ...
            if (ImGui::BeginTabItem("Settings")) {
                phynned_ui::draw_settings_panel();
                ImGui::EndTabItem();
            }
...
int main()
{
    // ── Auto-spawn phynned-agent if it isn't already running ───────────────────
    ...
    phynned_ui::AgentLauncher launcher;
    if (!launcher.start()) {
        std::fprintf(stderr,
            "[phynned-ui] Could not auto-launch phynned-agent - UI will run in "
            "disconnected mode until the agent is started manually.\n");
    }
    g_launcher = &launcher;

    phyriad::ui::ApplicationConfig cfg;
    cfg.window.title         = "Phynned - Runtime Optimizer";
    ...
    return phyriad::ui::Application::run(
           ...
           phyriad::render::FrameArena&     arena)
        {
            ...
            auto* logic = new PhynnedLogicNode();
            g_logic = logic;  // expose to draw_widgets for PhynnedClient access
            ...
            reg.register_factory(
                ...
                phyriad::schema::schema_hash<PhynnedAppState>(),
                phyriad::schema::schema_hash<phyriad::ui::InputEvent>()
            );

            reg.wire_registry().register_type<PhynnedAppState>();

            // ── RenderNode<PhynnedAppState> ─────────────────────────────────────
            using RN = phyriad::ui::RenderNode<PhynnedAppState>;
