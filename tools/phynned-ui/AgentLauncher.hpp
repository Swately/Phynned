// tools/phynned-ui/AgentLauncher.hpp
// AgentLauncher — detect-and-spawn helper so users only need to launch
// phynned-ui.exe; the background agent process appears automatically.
//
// Behaviour matrix:
//
...
//
//   Agent NOT running, spawn OK    → CreateProcess of phynned-agent.exe
...
public:
    ...
    [[nodiscard]] static bool agent_running() noexcept {
#ifdef _WIN32
        HANDLE h = OpenMutexW(SYNCHRONIZE, FALSE, L"Local\\PhynnedAgentMutex.v1");
        if (h) {
            CloseHandle(h);
            return true;
        }
        return false;
#else
        // POSIX: agent uses flock on /tmp/phynned-agent.lock. We attempt a
        // non-blocking lock to check; if it succeeds we release immediately.
        int fd = ::open("/tmp/phynned-agent.lock", O_CREAT | O_RDWR, 0666);
    ...
    // Windows or /proc/self/exe on Linux — we then look for phynned-agent
    ...
    [[nodiscard]] bool start() noexcept {
        ...
        if (n == 0u || n >= MAX_PATH) {
            std::fprintf(stderr,
                "[phynned-ui] GetModuleFileNameA failed (GLE=%lu)\n",
                GetLastError());
            return false;
        }
        const std::string agent_path = find_agent_exe_(ui_path, "phynned-agent.exe");
        if (agent_path.empty()) {
            std::fprintf(stderr,
                "[phynned-ui] agent launcher: could not locate phynned-agent.exe "
                "(searched: next to UI, ../phynned-agent/, parent dir/bin/)\n");
            return false;
        }
        ...
        if (!job_) {
            std::fprintf(stderr,
                "[phynned-ui] CreateJobObjectW failed (GLE=%lu)\n",
                GetLastError());
            return false;
        }
        ...
        {
            std::fprintf(stderr,
                "[phynned-ui] SetInformationJobObject failed (GLE=%lu)\n",
                GetLastError());
                ...
                &si, &pi))
        {
            ...
            std::fprintf(stderr,
                "[phynned-ui] CreateProcessA failed for '%s' (GLE=%lu)\n",
        ...
        if (!AssignProcessToJobObject(job_, pi.hProcess)) {
            std::fprintf(stderr,
                "[phynned-ui] AssignProcessToJobObject failed (GLE=%lu) — "
        ...
        return true;
#else
        // POSIX: posix_spawn phynned-agent located via the same multi-candidate
        // search the Windows side uses.
        char ui_path[4096] = {};
        const ssize_t n = readlink("/proc/self/exe", ui_path, sizeof(ui_path) - 1);
        if (n <= 0) return false;
        ui_path[n] = '\0';
        const std::string agent_path = find_agent_exe_(ui_path, "phynned-agent");
...
private:
    ...
    //        Canonical distribution layout: phynned-ui.exe
    ...
    //   3. <ui-dir>/../phynned-agent/<name>
    //        Legacy split-build-tree layout, where each tool had its
    //        own build subdirectory.
    //   4. <ui-dir>/../bin/<name>
    //        Hypothetical future layout where phynned-ui sits next to its
    //        resources (e.g. /opt/phynned/ui/) and binaries live under
    //        /opt/phynned/bin/.
    //
        ...
        const char* ui_path, const char* agent_name) noexcept
    {
        ...
        std::string candidates[] = {
            ...
            ui_dir + ".." + sep + "phynned-agent" + sep + agent_name, // (3) dev tree
