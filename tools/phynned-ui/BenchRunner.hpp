// tools/phynned-ui/BenchRunner.hpp
...
//   2. phynned-cli child process for CSV import (PresentMon CSV → phynned-bench)
//   3. Agent IPC pause/resume between baseline/treated runs (5-run protocol)
//   4. phynned-cli bench multi for the final statistical aggregation
//
...
struct BenchRun {
    ...
    char        bench_csv_path[260];  ///< output of phynned-cli presentmon-import
...
public:
    enum class State : uint8_t {
        ...
        ImportingRun     = 2,  ///< phynned-cli presentmon-import child active
        BetweenRuns      = 3,  ///< Cooldown between runs (also waits for cmd ack)
        AggregatingStats = 4,  ///< phynned-cli bench multi running
        Done             = 5,
        Failed           = 6,
    };
    ...
    /// Path to phynned-cli.exe. The UI looks for it next to itself.
    void set_phynned_cli_path(const char* path) noexcept {
    ...
    void set_client(ipc::PhynnedClient* c) noexcept { client_ = c; }
    ...
    /// Output of the final `phynned-cli bench multi` run, captured to a file
...
private:
    ...
    ipc::PhynnedClient*    client_{nullptr}; // for IPC commands (optional)
    ...
    // Subprocess for import (phynned-cli presentmon-import)
#ifdef _WIN32
                                      ...
                                      uint32_t    duration_sec) noexcept
{
    ...
    if (!pm_exe_[0] || !cli_exe_[0]) {
        std::snprintf(last_error_, sizeof(last_error_),
                      "PresentMon and phynned-cli paths must be set");
        return false;
    }
...
inline void BenchRunner::cancel() noexcept {
    ...
    if (protocol_ == Protocol::ABABA5Run && client_
        ...
        (void)client_->send_command(ipc::kPhynnedCmdResumePolicies);
    }
    state_           = State::Idle;
...
inline bool BenchRunner::spawn_import_(const BenchRun& r) noexcept {
    ...
    if (!CreateProcessW(nullptr, wcmd, nullptr, nullptr, FALSE,
        ...
        std::snprintf(last_error_, sizeof(last_error_),
                      "CreateProcess(phynned-cli import) failed");
        return false;
    }
...
inline void BenchRunner::poll_import_() noexcept {
    ...
    if (w == WAIT_OBJECT_0) {
        ...
        if (code != 0u) {
            std::snprintf(r.last_error, sizeof(r.last_error),
                          "phynned-cli presentmon-import exited %lu",
...
inline bool BenchRunner::send_policy_command_for_next_run_() noexcept {
    ...
    const uint32_t cmd = next.is_baseline
        ? ipc::kPhynnedCmdPausePolicies
        : ipc::kPhynnedCmdResumePolicies;
...
#ifdef _WIN32
    // Build: phynned-cli bench multi --baseline runX.bench.csv ... --treated runY.bench.csv ...
