// tools/phynned-ui/ReportExporter.hpp
// ReportExporter — generates a .md empirical report from BenchRunner results.
//
// Template mirrors the existing reports in docs/phynned/reports/. The exporter
...
/// Example: `<repo-root>/docs/phynned/reports/Fallout4_2026-05-17_7950x3d.md`
///
                                 ...
                                 uint32_t    out_cap) noexcept
{
    ...
    // Locate the phynned-ui.exe directory, climb to the project root if possible.
    // The dist layout puts the exe at: <root>/build/phynned-dist/phynned-ui.exe
    // and the reports live at:         <root>/docs/reports/
    // So from phynned-ui.exe: ../../docs/reports/  (installed copies fall
    // through to the %TEMP%\phynned-reports fallback below).
    char base_dir[260]{};
#ifdef _WIN32
    {
    ...
    if (base_dir[0] == '\0') {
        ...
        // phynned-reports/. Previously this was hardcoded to a dev
        ...
        if (tn > 0u && tn < sizeof(tmp)) {
            std::snprintf(base_dir, sizeof(base_dir),
                          "%sphynned-reports", tmp);
            CreateDirectoryA(base_dir, nullptr);  // OK if exists
        } else {
            std::strncpy(base_dir, ".\\phynned-reports", sizeof(base_dir) - 1);
        }
#else
        std::strncpy(base_dir, "./phynned-reports", sizeof(base_dir) - 1);
#endif
                                ...
                                const char*      output_path) noexcept
{
    ...
    out << "Pause/resume between baseline and treated runs via Phynned's IPC\n";
    out << "command slot.\n\n";
    ...
    // ── Aggregate log ─────────────────────────────────────────────────────
    out << "## Aggregate verdict (from `phynned-cli bench multi`)\n\n";
    out << "```\n";
    ...
    for (uint32_t i = 0u; i < n_runs; ++i) {
        ...
        out << "- " << runs[i].bench_csv_path << " (phynned-bench format)\n";
    }
    ...
    // ── Footer ────────────────────────────────────────────────────────────
    out << "---\n\n";
    out << "**Generated by**: `phynned-ui` Report Exporter.\n";
