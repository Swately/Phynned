@echo off
:: build-release.bat — distributable release build of Ayama (LTO + stripped).
:: Output: build-release\ayama-dist\ayama-ui.exe + runtime\*.exe
:: For the full release zip (staging + docs + install.ps1) use
:: scripts\package_release.ps1 -Version X.Y.Z -Strip -LTO instead.
setlocal
pushd "%~dp0"

cmake -S . -B build-release -DCMAKE_BUILD_TYPE=Release -DAYAMA_BUILD_TESTS=OFF ^
...
echo.
echo [OK] build-release\ayama-dist\ayama-ui.exe
popd
endlocal
