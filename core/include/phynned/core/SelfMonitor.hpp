// core/include/phynned/core/SelfMonitor.hpp
// SelfMonitor — Phynned-agent self-resource accounting.
//
...
public:
    ...
    void sample(WorkloadState state, float current_tick_ms) noexcept {
        ...
        if (!cur_.budget_ok && prev_budget_ok_) {
            std::fprintf(stderr,
                "[Phynned][SelfMonitor] Budget exceeded: CPU=%.2f%% (max %.2f%%) "
            ...
            std::fprintf(stdout,
                "[Phynned][SelfMonitor] Budget recovered: CPU=%.2f%% "
                "RSS=%llu MB\n",
