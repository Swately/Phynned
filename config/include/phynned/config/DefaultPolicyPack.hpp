// config/include/phynned/config/DefaultPolicyPack.hpp
...
// so that:
//   1. Users can inspect/edit what Phynned is doing.
...
// Privilege: None (file write to %LOCALAPPDATA%\Phynned\).
//
#pragma once
...
public:
        ...
        const phyriad::HardwareTopology& topo) noexcept
    {
        ...
        std::fclose(f);
        std::fprintf(stdout,
            "[Phynned] Default policy pack written for CPU class '%s': %s\n",
            sel.class_name(), path);
        return true;
    }
...
private:
                             ...
                             policy::CpuClass cpu_class) noexcept
    {
        ...
        std::fprintf(f,
            "# Phynned default policy pack — auto-generated on first run.\n"
            "# Edit this file to customise Phynned's behaviour.\n"
                            ...
                            policy::CpuClass cpu_class) noexcept
    {
        ...
        default:
            ...
            std::fprintf(f,
                ...
                "# less than 2%% improvement on this topology. Phynned will monitor\n"
