// action/include/phynned/action/AuditLog.hpp
...
// runs `phynned-cli memory clear-audit` (not yet implemented).
//
// Default path (resolved by AgentRuntime via ConfigStore):
//   Windows:  %LOCALAPPDATA%\Phynned\audit.bin
//   Linux:    ~/.config/phynned/audit.bin
//
...
public:
    ...
    /// fully independent from PhynnedAgentPublisher's cursor.
